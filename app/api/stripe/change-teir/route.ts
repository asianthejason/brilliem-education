...
export async function POST(req: Request) {
  ...
  // Downgrade to free = cancel at period end (you keep access through the period)
  if (desired === "free") {
  ...
  // Ensure any previously scheduled change is released so this request wins.
  ...
  // Paid -> paid only
  const currentPaid: Tier = currentTier === "free" ? "free" : currentTier;
  if (currentPaid === "free") {
  ...
  const currentRank = rank(currentPaid);
  ...
  // DOWNGRADE: take effect next billing period (no proration)
  ...
  const latestInvoice = updated.latest_invoice as any | null;
  const pi = latestInvoice?.payment_intent as any | null;

  // If Stripe needs immediate payment / 3DS, return PI secret to complete in Payment Element
  ...
  // Auto-paid or no additional charge -> update Clerk immediately
  const nextUnsafe: Record<string, any> = {
    ...meta,
    tier: desired,
    pendingTier: undefined,
