...
- Do NOT prefix any step with "Final answer:" or "Answer:". Just state the result plainly.
` +
      `- final_answer MUST be identical to the content of the LAST step (including all solutions and units). Do NOT include the phrase "Final answer" anywhere.
` +
...
` +
    ...
    // Strip any "Final answer:" prefix if the model includes it (we don't want a separate final-answer step label).
    steps = steps.map((s) => String(s).replace(/^\s*Final\s*answer\s*:\s*/i, "").trim()).filter(Boolean);


    // Use the steps as the source of truth.
    // Otherwise, science/history identifiers like "RDS-1" can be incorrectly reduced to "-1".
    // We do NOT try to extract a single numeric "final answer" from the steps.
// The steps themselves are the source of truth (and may legitimately contain multiple solutions).
// Keep finalAnswer in the API for backward compatibility, but set it to the last step text (without a "Final answer:" prefix).
const lastStep = steps.length ? steps[steps.length - 1] : "";
if (lastStep) {
  finalAnswer = normalizeTutorOutput(lastStep.replace(/^\s*Final\s*answer\s*:\s*/i, "").trim());
}
finalAnswer = dedupeTrailingUnits(finalAnswer);
    ...
    const result: ApiOk["result"] = {
      ...
      subject,
      lessons,
      // Don’t echo a separate final-answer UI element; the client renders only the steps.
